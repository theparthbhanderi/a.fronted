...
const DynamicQRCode = ({ data }: { data: CardData }) => {
    ...
    // Construct Aadhaar-style XML string using form data
    const qrString = `<?xml version="1.0" encoding="UTF-8"?>
<PrintLetterBarcodeData uid="${data.idNumber.replace(/\D/g, '')}" name="${data.nameEnglish}" gender="${data.gender.charAt(0).toUpperCase()}" yob="${yearOfBirth}" address="${cleanAddressInfo}" />`;

    return (
        <div style={{ width: 100, height: 100, background: '#fff', padding: 2 }}>
            <QRCodeSVG value={qrString} size={96} level="M" includeMargin={false} />
        </div>
    );
};
...
const FrontCard = ({ data, photoSrc }: { data: CardData; photoSrc: string | null }) => (
    <div style={{
        width: 420, height: 275, background: '#fff',
        ...
        <img
            src="https://img.sanishtech.com/u/4c908f9f2b190bba7425f2e9b764acd4.png"
            alt="Front Header"
            style={{ width: '100%', height: 46, objectFit: 'fill', display: 'block', flexShrink: 0 }}
        />
        ...
        <div style={{ display: 'flex', padding: '5px 12px 0 8px', flex: 1 }}>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', paddingLeft: 12, paddingTop: 1 }}>
                ...
                <div style={{
                    ...
                    <div style={{ fontSize: 8, color: '#000', lineHeight: 1.25 }}>
                    ...
                    <div style={{ fontSize: 8, color: '#000', lineHeight: 1.25, marginTop: 1.5 }}>
        ...
        <div style={{ textAlign: 'center', flexShrink: 0, marginTop: '-3px', marginBottom: '2px' }}>
            <div style={{ fontSize: 24, fontWeight: '900', color: '#000', letterSpacing: 1.5 }}>
                {fmt12(data.idNumber)}
            </div>
        </div>
        ...
        <img
            src="https://img.sanishtech.com/u/d16a09cb4325086efa8cb136fc64ff34.png"
            alt="Front Footer"
            style={{ width: '100%', height: 38, objectFit: 'fill', display: 'block', flexShrink: 0 }}
        />
    </div>
);
...
const BackCard = ({ data }: { data: CardData }) => (
    <div style={{
        ...
        {/* Header (Fixed Height) */}
        <img
            src="https://img.sanishtech.com/u/abc30d3aaa22ce201f9f34af5f06feca.png"
            alt="Back Header"
            style={{ width: '100%', height: 46, objectFit: 'fill', display: 'block', flexShrink: 0 }}
        />
        ...
        <div style={{ textAlign: 'center', marginBottom: 4, flexShrink: 0 }}>
            <div style={{ fontSize: 20, fontWeight: '900', color: '#000', letterSpacing: 1.2 }}>
                {fmt12(data.idNumber)}
            </div>
            {/* Thin line exactly like PDF */}
            <hr style={{ borderTop: '0.5px solid #000', margin: '2px auto', width: '230px' }} />
            <div style={{ fontSize: 12, fontWeight: 'bold', color: '#000', marginTop: 2 }}>
                VID : {fmt16(data.vid)}
            </div>
        </div>

        {/* Back Footer Wrapper to add the red top border like original */}
        <div style={{ height: 22, width: '100%', borderTop: '1px solid #cc3333', flexShrink: 0 }}>
            <img
                src="https://img.sanishtech.com/u/801cbd86a3a391491065593071ababbe.png"
                alt="Back Footer"
